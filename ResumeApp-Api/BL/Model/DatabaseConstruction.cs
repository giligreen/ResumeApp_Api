...
namespace BL
{
    static class DatabaseConstruction
    {
        ...
        /// ומחזירה מערך של מילים לאחר ניקוי והסרת פיצ'רים- מילים 
        /// </summary>
        ...
        public static List<string> DividingFileIntoWords(string filePath)
        {
            ...
            //הסרת אותיות בודדות
            words.RemoveAll((item) => item.Length <= 1);
            //הסרת מילים אוטומטיות שנוספו בעת ההמרה לקובץ טקסט
            words.RemoveRange(0, 10);
            words.RemoveRange(words.Count() - 22 - 1, 23);

            //   מילות עצירה - StopWords הסרת
            string[] StopWords = GetStopWords();
            foreach (var item in StopWords)
            {
                words.RemoveAll(x => x == item);
            }
